...
def main():
    ...
    # resample_wav
    parser.add_argument("--sample-rate", type=int, default=22050, help="Resample WAV to this rate (e.g., 22050)")
    #
    ...
    def __call__(
        ...
        try:
            ...
            if (not args.skip_existing_wav) or (
                ...
                # resample_wav
                resample_wav(wav_path, args.sample_rate)
...
def resample_wav(wav_path: Path, sample_rate: int):
    if sample_rate is None:
        return
    temp_path = wav_path.with_suffix(".tmp.wav")
    command = ["ffmpeg", "-i", str(wav_path), "-ar", str(sample_rate), "-y", str(temp_path)]
